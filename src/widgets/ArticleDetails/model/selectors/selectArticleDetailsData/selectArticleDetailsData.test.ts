...
describe('widgets/ArticleDetails/selectArticleDetailsData', () => {
  test('Проверка положительного результата', () => {
    const article: Article = {
      ...
      title: 'Как разработать Telegram-бота для генерации сложных паролей',
      subtitle: 'Что нового в JS за 2022 год?',
      image: 'https://doprof.ru/upload/medialibrary/a4c/javscript.jpg',
      views: 12843,
      createAt: '21.02.2022',
      type: ['IT'],
      blocks: [
    ...
    const articleData = selectArticleDetailsData(state as StateSchema);
    expect(articleData).toEqual(article);
  });

  test('Проверка на пустой state', () => {
    const state: DeepPartial<StateSchema> = {};
    const articleData = selectArticleDetailsData(state as StateSchema);
    expect(articleData).toEqual(undefined);
  });
});
